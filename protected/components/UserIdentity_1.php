	...
	public function authenticate()
	{
		$user = User::model()->findByAttributes(array('username'=>$this->username));	
			...
			$companiesdb = Usercompany::model()->findByAttributes(array('userid'=>$user->userid,'active'=>1));
                        ...
                        $firstname = ($user->employeeuser->firstname)?$user->employeeuser->firstname:$user->customeruser->firstname;
                        $plastname = ($user->employeeuser->plastname)?$user->employeeuser->plastname:$user->customeruser->plastname;
                        $this->setState('userid',$user->userid);
                        $this->setState('profileid',$user->profileid);
                        $this->setState('profiledsc',$profile->profiledsc);
			$this->setState('username',$firstname." ".$plastname);
