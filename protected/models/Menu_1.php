                        ...
                        foreach( $result as $row){
                            if($row->level==0){
                               ...
                               $maynus[0][$row->menudsc] = array(
                                            'state'=> 'app.'.str_replace(" ", "_", $descrip), 
                                            'translate'=> 'aside.nav.'.str_replace(" ", "_", $descrip), 
                                            'url'=> "/".$descrip, 
                                            'templateUrl'=>'index.php?r='.$row->menulink,
                                            'menuid'=> $row->menuid,
                                            'level'=> $conta,
                                            'icon'=>$row->icon,
                                            'ml'=>$ml
                                    ...
                                    if($submenu->menuparentid==$row->menuid){
                                        if($submenu->menudsc==$row->menudsc){ $flag=1; }else{ $flag=0;}
                                            $maynus[1][$submenu->menudsc] = array(
                                                'state'=> 'app.table.'. str_replace(" ", "_", $descrip1),
                                                'translate'=> 'aside.nav.'.str_replace(" ", "_", $descrip1),
                                                'url'=> $submenu->menulink, 
                                                'templateUrl'=> 'index.php?r='.$submenu->menulink,
